...
def tab_generate(info: dict) -> None:
    ...
    try:
        ...
        with chart_col1:
            fig_pie = go.Figure(data=[go.Pie(
                ...
                values=[result.summary.eligible_students, result.summary.not_eligible_students],
                hole=0.55,
            ...
            fig_pie.update_layout(
                title=dict(text="Eligibility Breakdown", font=dict(color="#93c5fd", size=15)),
        ...
        with chart_col2:
            room_names  = list(room_allocations_enriched.keys())
            room_counts = [len(df) for df in room_allocations_enriched.values()]
            room_caps   = [
                next((r["Capacity"] for r in rooms_with_info if r["Room Number"] == rn), len(df))
                for rn, df in room_allocations_enriched.items()
            ]
            fig_bar = go.Figure()
            fig_bar.add_trace(go.Bar(
                name="Capacity",
                x=room_names, y=room_caps,
                marker_color="#334155",
                text=room_caps, textposition="outside",
                textfont=dict(color="#94a3b8"),
            ))
            fig_bar.add_trace(go.Bar(
                name="Allocated",
                x=room_names, y=room_counts,
                marker_color="#3b82f6",
                text=room_counts, textposition="outside",
                textfont=dict(color="#93c5fd"),
            ))
            fig_bar.update_layout(
                title=dict(text="Room Occupancy", font=dict(color="#93c5fd", size=15)),
                barmode="overlay",
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                legend=dict(font=dict(color="#94a3b8"), bgcolor="rgba(0,0,0,0)"),
    ...
    # ── Room-wise seating preview ──────────────────────────────────────────────
    st.divider()
    st.subheader("🏫 Room-wise Seating Preview")
    for room_no, room_df in room_allocations_enriched.items():
        occupancy = len(room_df)
        cap = next((r["Capacity"] for r in rooms_with_info if r["Room Number"] == room_no), occupancy)
        pct = int(occupancy / cap * 100) if cap else 0
        with st.expander(f"Room {room_no}  —  {occupancy} / {cap} seats  ({pct}% full)", expanded=False):
            preview_cols = [c for c in ["Seat Number", "Roll Number", "Name", "Attendance Percentage", "Section", "Branch"] if c in room_df.columns]
            st.dataframe(room_df[preview_cols].reset_index(drop=True), use_container_width=True, hide_index=True)
    ...
    try:
        save_run(
            ...
            room_allocations=room_allocations_enriched,
            not_eligible_df=not_eligible_df,
        ...
        st.warning(f"Run saved to files but could not write to database: {_db_exc}")
...
[data-testid="stSidebar"] h2 { color: #60a5fa !important; font-size: 1rem !important; }
...
[data-testid="stTabs"] [role="tab"]:hover { background: #334155; color: #e2e8f0; }
...
[data-testid="metric-container"] [data-testid="stMetricLabel"] { color: #94a3b8 !important; font-size: 0.8rem !important; }
[data-testid="metric-container"] [data-testid="stMetricValue"] { color: #60a5fa !important; font-weight: 700 !important; font-size: 1.8rem !important; }
...
[data-testid="stButton"] > button:not([kind="primary"]):hover { border-color: #3b82f6; }
...
[data-testid="stDataFrame"] th { background: #1e3a5f !important; color: #93c5fd !important; }
...
[data-testid="stExpander"] summary { color: #93c5fd; font-weight: 600; }
...
.hero-banner .hero-icon { font-size: 3rem; }
.hero-banner h1 { margin: 0; font-size: 1.7rem; color: #f1f5f9 !important; }
.hero-banner p  { margin: 4px 0 0 0; color: #94a3b8; font-size: 0.9rem; }
...
def _hero_banner() -> None:
    st.markdown(
        ...
        <div class="hero-banner">
            ...
            <div>
                <h1>SIRT Exam Seating System</h1>
                <p>Automatic &bull; Attendance-Based &bull; SIRT Formatted Output &bull; One-Click Reports</p>
            </div>
        </div>
        """,
...
def tab_history() -> None:
    ...
    st.caption("All past seating allocation runs stored in the local SQLite database.")

    history = get_run_history()
    if history.empty:
        st.info("No runs yet. Generate a seating plan in **Step 2** to see history here.")
        return

    # Pretty display column rename
    display = history.rename(columns={
        ...
        "not_eligible_students": "Debarred",
        "allocated_students":   "Allocated",
        "room_count":           "Rooms",
    })[["Run ID", "Generated At", "Exam", "Semester", "Cutoff (%)",
        "Total", "Eligible", "Debarred", "Allocated", "Rooms"]]
    ...
    run_options = history["run_id"].tolist()
    selected_run = st.selectbox(
        "Select run ID",
        options=run_options,
        format_func=lambda r: (
            f"{r}  —  "
            + history.loc[history['run_id'] == r, 'created_at'].iloc[0]
            + "  —  "
            + str(history.loc[history['run_id'] == r, 'exam_title'].iloc[0])
        ),
    )

    if selected_run:
        alloc_df   = get_run_allocations(selected_run)
        debarr_df  = get_run_debarred(selected_run)

        a_tab, d_tab = st.tabs(["Seat Allocations", "Debarred Students"])

        with a_tab:
            st.caption(f"{len(alloc_df)} allocated seats across {alloc_df['room_number'].nunique() if not alloc_df.empty else 0} room(s)")
            if not alloc_df.empty:
                show_cols = [c for c in ["room_number", "seat_number", "roll_number",
                                         "student_name", "attendance_percentage",
                                         "section", "branch"] if c in alloc_df.columns]
                st.dataframe(alloc_df[show_cols], use_container_width=True, hide_index=True)
            else:
                st.info("No allocation data found for this run.")

        with d_tab:
            st.caption(f"{len(debarr_df)} debarred student(s)")
            if not debarr_df.empty:
                show_cols = [c for c in ["roll_number", "student_name",
                                         "attendance_percentage", "section", "branch"]
                             if c in debarr_df.columns]
                st.dataframe(debarr_df[show_cols], use_container_width=True, hide_index=True)
            else:
                st.info("No debarred students for this run.")

        st.divider()
        if st.button("🗑️ Delete this run from database", type="secondary"):
            delete_run(selected_run)
            st.success(f"Run {selected_run} deleted.")
            st.rerun()
